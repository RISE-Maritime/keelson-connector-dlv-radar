...
class OD4Session:
    ...
    def __process(self, e):
        logging.debug("Received Envelope with ID = %s/%s" % ((e.dataType), str(e.senderStamp)))
    ...
    def __runner(self):
        ...
        while True:
            ...
            if len(data) != 65536:
                # Avoid corrupt packets. - This assumes all packets will be smaller than the len(data) size, which is a really bad way of doing this. 
                buf =  b"".join([buf, data])
